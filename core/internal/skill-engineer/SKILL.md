...
- **Submodule-Awareness:** Wenn du in einer Umgebung mit Git-Submodulen arbeitest (z.B. im Skill-Evolution Projekt) und an einem Skill im Submodule-Verzeichnis (z.B. innerhalb von `skills/`) etwas änderst, führe IMMER einen Zwei-Phasen-Commit aus: 1. Im Submodule committen (`cd skills`). 2. Den Pointer im Haupt-Repo aktualisieren (`cd ..`).
