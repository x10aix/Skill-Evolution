...
* `tbd` - **feat**: **Stateful Ecosystem Pruning** — Ecosystem Mapping is now optional and will not block imports from external systems like OpenClaw.
* `tbd` - **feat**: **Fairness & Credits (Modus C)** — When auditing or editing external skills, the agent proactively offers to append an `## Origin / Credits` block to properly attribute the original authors of the forked code without enforcing it.
